...
export class EscolasComponent implements OnInit {
  ...
  ngOnInit() {
    this.data.getEscolas().subscribe(list => {
      this.escolas = list;
      this.applyFilter();
    });
    this.buildForm();
  ...
  save() {
    ...
    this.data.saveEscola(payload).subscribe({
  ...
  toggleStatus(item: Escola) {
    const obs = item.status === 'ATIVO' ? this.data.inativarEscola(item.id) : this.data.ativarEscola(item.id);
